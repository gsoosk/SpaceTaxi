#include "SpaceTaxi.h"
using namespace std;
void SpaceTaxi:: register_passenger(string Username, string Password,string PhoneNumber, bool UseDiscountCode, string DiscountCodeStr)
{
	try{
		...
		if(UseDiscountCode)
			this->use_discount_code(DiscountCodeStr);
	}
...
void SpaceTaxi:: register_driver(string Username, string Password, string SpaceShipNumber, string SpaceShipModel, SpaceShip newSpaceShip, bool VIP)
{
...
void SpaceTaxi:: accept_registration(std::string &Username)
{
	try{
...
void SpaceTaxi:: reject_registration(std::string &Username)
{
	try{
		if(!this->does_it_loged_in(ourAdmin.get_username()))
			throw BadException("Admin doesnt loged in\n");
...
void SpaceTaxi:: show_registration_requests()
{
	try{
		if(!this->does_it_loged_in(ourAdmin.get_username()))
			throw BadException("Admin doesnt loged in\n");
...
void SpaceTaxi:: get_discount_code(std::string &Username)
{
	try{
		...
		for(int i=0; i<RegisteredPassengers.size(); i++)
		{
			if(Username==RegisteredPassengers[i]->get_username())
			{
				...
				RegisteredPassengers[i]->print_discount_code();
			}
		}
	}
	catch(BadException ex)
...
void SpaceTaxi:: use_discount_code(string CommandLine)
{
...
void SpaceTaxi:: set_status_available(string Address,string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again\n");
...
void SpaceTaxi:: set_status_unavailable(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again\n");
...
void SpaceTaxi:: request_trip(string SourceAddress, string Username, Trip* RequestedTrip, bool VIP)
{
	try{
...
void SpaceTaxi:: cancel_trip_request(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again\n");
		...
		if(TripPassenger==NULL)
			throw BadException("There is'nt any loged in user with this username\n");
		TripPassenger->cancel_trip_request();
	}
	catch(BadException ex)
	{
		ex.print_Exception();
...
void SpaceTaxi:: show_trip_requests(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again\n");
		...
		for(int i=0; i<RegisteredPassengers.size(); i++)
		{
			...
			{
				RegisteredPassengers[i]->print_trip_request();
			}
		}
	}
	catch(BadException ex)
...
void SpaceTaxi:: accept_trip_request(string DriverUsername, string PassengerUsername)
{
	try{
...
void SpaceTaxi:: trip_status(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again!\n");
		...
		{	
			...
			{
				cout<<"accepted ";
				OurPassenger->print_last_accepted_trip();
			}
		}
		else
...
void SpaceTaxi:: arrived(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again!\n");
...
void SpaceTaxi:: end_trip(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again!\n");
...
void SpaceTaxi:: rate_driver(string Username, int AllRate)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again!\n");
...
void SpaceTaxi:: charge_account(string Username, int Credit)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again!\n");
...
void SpaceTaxi:: get_credit(string Username)
{
	try{
		if(!this->does_it_loged_in(Username))
			throw BadException("Please login and then try again!\n");
...
void SpaceTaxi:: set_time(int Day, int Hour, int Minute, int Second)
{
	try{
		if(!this->does_it_loged_in(ourAdmin.get_username()))
			throw BadException("Admin doesnt loged in!\n");
...
void SpaceTaxi:: driver_report(string DriverUsername, Date FirstDate, Date SecondDate)
{
	try{
		if(!this->does_it_loged_in(ourAdmin.get_username()))
			throw BadException("Admin doesnt loged in!\n");
		Driver* OurDriver = this->find_driver(DriverUsername);
		OurDriver->driver_report(FirstDate, SecondDate);
	}
	catch(BadException ex)
	{
		ex.print_Exception();
...
void SpaceTaxi:: passenger_report(string PassengerUsername)
{
	try{
		if(!this->does_it_loged_in(ourAdmin.get_username()))
			throw BadException("Admin doesnt loged in!\n");
...
void SpaceTaxi:: usual_system_report()
{
	...
	cout<<SystemCredit<<" "<<TripsCount<<" "<<DriversCount<<" "<<PassengersCount<<endl;
}
void SpaceTaxi:: system_report_by_time(Date FirstDate, Date SecondDate)
{
